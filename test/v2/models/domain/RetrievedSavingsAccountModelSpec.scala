...
class RetrievedSavingsAccountModelSpec extends UnitSpec with JsonErrorValidators {

  val model = RetrievedSavingsAccount("Bank Account 1")
  ...
  "reads" when {
    "single account returned from des" should {
      "read to model" in {
        singleJsonFromDesArray.as[List[RetrievedSavingsAccount]] shouldBe List(model)
      }
    }
    ...
    testMandatoryProperty[RetrievedSavingsAccount](singleJsonFromDes)("/incomeSourceName")

    testPropertyType[RetrievedSavingsAccount](singleJsonFromDes)(
