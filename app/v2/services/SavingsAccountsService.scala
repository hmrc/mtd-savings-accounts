...
class SavingsAccountsService @Inject()(connector: DesConnector) extends DesServiceSupport {
  ...
  private def desErrorToMtdErrorRetrieveAll: Map[String, Error] = Map(
    ...
    logger.warn(s"[SavingsAccountsService] [retrieveAll] - Unexpected DES error returned - No mapping found for error code $error")
    DownstreamError
  }
  ...
  private def desErrorToMtdErrorRetrieve: Map[String, Error] = Map(
    ...
    logger.warn(s"[SavingsAccountsService] [retrieve] - Unexpected DES error returned - No mapping found for error code $error")
    DownstreamError
  }
